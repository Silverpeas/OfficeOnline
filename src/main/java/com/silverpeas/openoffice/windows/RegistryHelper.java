...
public class RegistryHelper extends OpenOfficeFinder {
  ...
  private static final String[] VERSIONS = new String[]{"3.1", "3.0", "2.4", "2.3"};

  private static final String GLOBAL_OPEN_OFFICE_FOLDER = "\"HKEY_LOCAL_MACHINE\\SOFTWARE\\OpenOffice.org\\OpenOffice.org\\";
  private static final String LOCAL_OPEN_OFFICE_FOLDER = "\"HKEY_CURRENT_USER\\SOFTWARE\\OpenOffice.org\\OpenOffice.org\\";
  ...
  public String findOpenOffice() throws OpenOfficeNotFoundException {
    for (String version : VERSIONS) {
      logger.log(Level.FINE, "Looking for open office version : " + version);
      String result = getOpenOfficePath(version);
      if (result != null) {
        logger.log(Level.FINE, "Open office version " + version + " found");
        return '"' + result + '"';
      }
    }
